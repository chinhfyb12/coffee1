...
function Cart(props) {
  ...
  useEffect(() => {
    ...
    if(products[0]) {
      ...
      props.sendProductsToCheckout(products)
    }
  }, [products]);

  function getQuantityChange(productChange) {
    const listTemp = [...products];
    const index = listTemp.findIndex((item) => item.keyProduct === productChange.keyProduct);
    if (index < 0) return;
    if (listTemp) {
      listTemp[index].quantity = productChange.quantity;
      setProducts(listTemp);
      props.updateProduct(listTemp)
    }
  }
  function getProductDelete(keyProduct) {
    const index = products.findIndex((item) => item.keyProduct === keyProduct);
    if (index < 0) return;
    ...
    <>
      <div className="row box-cart">
        <div className="col-8 box-cart__products">
          ...
          <ul className="list-title d-flex">
            <li className="nav-item">Sản phẩm</li>
            <li className="nav-item">Giá</li>
        ...
        <div className="col-4 box-total">
          ...
          <p className="note">
            <i>Mua hàng và thuế ở danh mục thanh toán</i>
          </p>
          <Link onClick={ handlePayment } to="/checkout" className="btn">
            THANH TOÁN
          </Link>
          <p className="svg">
            <svg
...
const mapDispatchToProps = dispatch => {
  return {
    sendProductsToCheckout: list => dispatch({type: "SEND_TO_CHECKOUT", list}),
